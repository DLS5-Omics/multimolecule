...
from multimolecule.data import Dataset, DatasetRegistry
...
@RunnerRegistry.register("multimolecule", default=True)
class Runner(dl.Runner):
    ...
    def train(self, train_splits: list[str] | None = None, evaluate_splits: list[str] | None = None) -> NestedDict:
        ...
        Args:
            ...
            evaluate_splits: list of split to run evaluate.
                Defaults to splits in `self.dataloaders` except for those in `train_splits`.
    ...
    def _build_dataset(self, config: NestedDict, name: str | None = None) -> NestedDict:
        ...
        if is_hf_dataset:
            ...
            dataset = self._build_local_dataset(config, root, ratio, name)
        else:
            raise ValueError(
