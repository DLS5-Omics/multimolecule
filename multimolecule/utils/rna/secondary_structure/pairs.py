...
class PairMap:
    ...
    def segments(self) -> List[Segment]:
        if not self._data:
            return []
        return pairs_to_duplex_segment_list(self.pairs)
...
def _numpy_pairs_to_duplex_segment_list(pairs: np.ndarray) -> List[Segment]:
    pairs = np.asarray(pairs, dtype=int)
    if pairs.size == 0:
        return []
    if pairs.ndim != 2 or pairs.shape[1] != 2:
    ...
    if positions.size == 1:
        if left_mask[0]:
            return [[(int(positions[0]), int(partner_pos[0]))]]
        return []
    ...
    for seg_start, seg_len in zip(start_idx.tolist(), lengths.tolist()):
        seg_positions = positions[seg_start : seg_start + seg_len]
        seg_partners = partner_pos[seg_start : seg_start + seg_len]
        segments.append([(int(pi), int(pj)) for pi, pj in zip(seg_positions, seg_partners)])
    return segments
