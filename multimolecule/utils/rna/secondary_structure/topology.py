...
class RnaSecondaryStructureTopology(UndirectedGraph):
    ...
    def tiers(self) -> List[Tier]:
        if len(self) == 0:
            return []
        pair_tiers = StemSegments._tier_pairs(self._all_pairs)
        out: List[Tier] = []
    ...
    def tier_stem_segments(self, level: int) -> StemSegments:
        return self._tier_at_level(level).stem_segments
    ...
    def tier_helix_segments(self, level: int) -> HelixSegments:
        return self._tier_at_level(level).helix_segments
    ...
    def tier_stem_edges(self, level: int) -> List[StemEdge]:
        return self._tier_at_level(level).stem_segments.edges
    ...
    def tier_helix_edges(self, level: int) -> List[StemEdge]:
        return self._tier_at_level(level).helix_segments.edges

    @cached_property
    def all_helix_edges(self) -> List[StemEdge]:
        return [edge for segments in self.all_helix_segments for edge in segments.edges]

    def tier_stem_graph(self, level: int) -> DirectedGraph:
        return self._tier_at_level(level).stem_segments.graph

    @cached_property
    def all_stem_graphs(self) -> List[DirectedGraph]:
        return [stems.graph for stems in self.all_stem_segments]

    def tier_helix_graph(self, level: int) -> DirectedGraph:
        return self._tier_at_level(level).helix_segments.graph
    ...
    @cached_property
    def taxonomy_loop_segments(self) -> List[LoopSegments]:
        return self.loop_segments_by_tier(view=StructureView.ALL)
    ...
    def loop_segment_contexts(
        ...
        *,
        ...
        if view is None:
            ...
            return self._tier_at_level(level).loop_segments.contexts(self._pseudoknot_pairs)
        view = StructureView.parse(view)
        if view == StructureView.NESTED:
            return self.nested_loop_segments.contexts(self._pseudoknot_pairs)
        raise ValueError(
    ...
    def partner_at(
        ...
        *,
        ...
        if level is None:
            partner = int(self._partner_map_all[pos].item())
            return None if partner < 0 else partner
        pair_map = self._partner_map_from_pairs(self._tier_at_level(level).pairs, len(self))
        partner = int(pair_map[pos].item())
        return None if partner < 0 else partner
    ...
    def stem_at(
        ...
        *,
        ...
        if level is None:
            return self._stem_pair_map.get(pair)
        return self.tier_stem_segments(level).pair_map.get(pair)
    ...
    def helix_at(
        ...
        *,
        ...
        if level is None:
            return self._helix_pair_map.get(pair)
        return self.tier_helix_segments(level).pair_map.get(pair)
    ...
    def _tier_at_level(self, level: int) -> Tier:
...
class Tier:
    ...
    @property
    def stem_segments_regions(self) -> List[StemSegment]:
        return self.stem_segments.segments

    @property
    def helix_segments_regions(self) -> List[HelixSegment]:
        return self.helix_segments.segments

    @property
    def stem_graph(self) -> DirectedGraph:
        return self.stem_segments.graph

    @property
    def helix_graph(self) -> DirectedGraph:
        return self.helix_segments.graph

    @property
    def stem_edges(self) -> List[StemEdge]:
        return self.stem_segments.edges

    @property
    def helix_edges(self) -> List[StemEdge]:
        return self.helix_segments.edges
...
class Loops:
    ...
    def _from_pairs_list(
        ...
        *,
        ...
        partner_by_pos = cls._build_partner_map(pairs_list, length)
        ...
        for outer_pair, (segments, anchor_pairs) in face_groups.items():
            anchor_helices = sorted(
                {pair_to_helix[pair] for pair in anchor_pairs if pair in pair_to_helix},
                key=lambda segment: (
                    segment.tier,
                    segment.start_5p,
                    segment.start_3p,
            ...
            loop_anchor_pairs = tuple(sorted(anchor_pairs))
            loops.append(
                Loop(
                    kind,
                    loop_spans,
    ...
    def taxonomy_from_pairs(
        ...
        return cls._taxonomy_from_pairs_list(ensure_pairs_list(pairs), length)

    @classmethod
    def _taxonomy_from_pairs_list(cls, pairs_list: List[Pair], length: int) -> Loops:
        pk_type, h_type_stems, kissing, pk_span = cls._classify_pseudoknot(pairs_list, length)
        if pk_type == PseudoknotType.NONE:
            return cls._from_pairs_list(pairs_list, length, pseudoknot_type=PseudoknotType.NONE, is_nested=True)
        ...
        if pk_type in (PseudoknotType.M_TYPE, PseudoknotType.COMPLEX):
            topological = cls._from_pairs_list(
                pairs_list,
                length,
            ...
            if pk_span is None:
                return cls._from_pairs_list(pairs_list, length, pseudoknot_type=pk_type, is_nested=False)
            pk_loops: List[Loop] = []
        ...
        if pk_type != PseudoknotType.H_TYPE or h_type_stems is None:
            return cls._from_pairs_list(pairs_list, length, pseudoknot_type=PseudoknotType.UNKNOWN, is_nested=False)
        ...
        pair_to_helix = cls._helix_pair_map_by_tiers(pairs_list)
        pair_to_stem = StemSegments.pair_map_by_tiers(pairs_list)
        partner_by_pos = cls._build_partner_map(pairs_list, length)
        for role, loop_span in span_map.items():
            ...
            anchor_helices = tuple(
                sorted(
                    {pair_to_helix[pair] for pair in loop_anchor_pairs if pair in pair_to_helix},
                    key=lambda segment: (
                        segment.tier,
                        segment.start_5p,
                        segment.start_3p,
    ...
    def hairpins(self) -> List[Loop]:
        return self._filter(LoopType.HAIRPIN)

    def bulge_loops(self) -> List[Loop]:
        return self._filter(LoopType.BULGE)

    def internal_loops(self) -> List[Loop]:
        return self._filter(LoopType.INTERNAL)

    def multiloops(self) -> List[Loop]:
        return self._filter(LoopType.MULTILOOP)

    def external_loops(self) -> List[Loop]:
        return self._filter(LoopType.EXTERNAL)
    ...
    def _filter(self, kind: LoopType) -> List[Loop]:
        return [loop for loop in self._loops if loop.kind == kind]
    ...
    @staticmethod
    def _build_partner_map(pairs: List[Pair], length: int) -> List[int]:
        return PairMap(pairs, length=length).to_list()
    ...
    def _face_segments(
        ...
        half_to_face: Dict[Pair, int] = {}
        face_count = 0
        for u, v in edges:
            for half_edge in ((u, v), (v, u)):
                ...
                while cur not in visited:
                    visited.add(cur)
                    half_to_face[cur] = face_id
                    src, dst = cur
                    neighbors = rotation[dst]
                    idx = neighbors.index(src)
                    next_dst = neighbors[(idx + 1) % len(neighbors)] if len(neighbors) > 1 else neighbors[0]
                    cur = (dst, next_dst)
        ...
        for idx, partner in enumerate(partner_by_pos):
            ...
            face_key = half_to_face.get(edge)
            if face_key is None:
                continue
    ...
    def _classify_pseudoknot(
        ...
        if not crossing_pairs(pairs):
            return PseudoknotType.NONE, None, None, None
        h_type_stems = cls._h_type_pseudoknot_stems(pairs)
        if h_type_stems is not None:
            return PseudoknotType.H_TYPE, h_type_stems, None, None
        kissing = cls._kissing_hairpin_loops(pairs, length)
        if kissing is not None:
            return PseudoknotType.KISSING_HAIRPIN, None, kissing, None
        pk_type = cls._taxonomy_type_from_pairs(pairs, length)
        if pk_type in (PseudoknotType.M_TYPE, PseudoknotType.COMPLEX):
            stem_pairs = cls._crossing_stem_pairs(pairs)
    ...
    @classmethod
    def _h_type_pseudoknot_stems(cls, pairs: List[Pair]) -> Tuple[StemSegment, StemSegment] | None:
        crossing = crossing_pairs(pairs)
        if not crossing:
            return None
        pair_to_stem = StemSegments.pair_map_by_tiers(pairs)
        stems = {pair_to_stem[pair] for pair in crossing if pair in pair_to_stem}
        if len(stems) != 2:
            return None
        stem1, stem2 = sorted(stems, key=lambda stem: stem.start_5p)
    ...
    @classmethod
    def _crossing_stem_pairs(cls, pairs: List[Pair]) -> List[Tuple[HelixSegment, HelixSegment]]:
        if not pairs:
            return []
        pairs_tensor = torch.tensor(pairs, dtype=torch.long)
    ...
    @classmethod
    def _taxonomy_type_from_pairs(cls, pairs: List[Pair], length: int) -> PseudoknotType | None:
        if not pairs:
            return None
        if not crossing_pairs(pairs):
            return PseudoknotType.NONE
        stems = cls._h_type_pseudoknot_stems(pairs)
        if stems is not None:
            return PseudoknotType.H_TYPE
        kissing = cls._kissing_hairpin_signature(pairs, length)
        if kissing is not None:
            _nested_loops, _loop_indices, helix_count = kissing
            return PseudoknotType.KISSING_HAIRPIN if helix_count == 1 else PseudoknotType.COMPLEX
        stem_pairs = cls._crossing_stem_pairs(pairs)
        if not stem_pairs:
            return PseudoknotType.UNKNOWN
        pseudoknot_stems = {stem.key for pair in stem_pairs for stem in pair}
        if len(pseudoknot_stems) < 3:
            return PseudoknotType.UNKNOWN
        pairs_tensor = torch.tensor(pairs, dtype=torch.long)
        helix_segments = HelixSegments(pairs_tensor, 0).segments
        if not helix_segments:
            return PseudoknotType.UNKNOWN
        stem_indices = {stem.key: idx for idx, stem in enumerate(helix_segments)}
        try:
            pseudoknot_indices = {stem_indices[key] for key in pseudoknot_stems}
        except KeyError:
            return PseudoknotType.UNKNOWN
        paired_positions = StemSegments._paired_positions_from_pairs(pairs_tensor)
        _graph, edges = StemSegments._build_stem_graph_from_segments(
            helix_segments,
            paired_positions,
            tier=0,
        ...
        adjacency: Dict[int, Set[int]] = {idx: set() for idx in range(len(helix_segments))}
        for edge in edges:
            adjacency[edge.src].add(edge.dst)
            adjacency[edge.dst].add(edge.src)
        pk_adjacency = {idx: {nbr for nbr in adjacency[idx] if nbr in pseudoknot_indices} for idx in pseudoknot_indices}
    ...
    def _kissing_hairpin_signature(
        ...
        pairs: List[Pair],
        length: int,
    ) -> Tuple[Loops, Tuple[int, int], int] | None:
        nested_pairs, pseudoknot_pairs = split_pseudoknot_pairs(pairs)
        ...
        for idx in range(count):
            ...
            segments.append([(start_5p + offset, start_3p - offset) for offset in range(seg_len)])
        if not segments:
            return None
        segments = sorted(segments, key=lambda segment: (segment[0][0], segment[0][1]))
        helix_groups: List[List[List[Pair]]] = []
        current: List[List[Pair]] = [segments[0]]
        for segment in segments[1:]:
            prev = current[-1]
            prev_end_i, prev_end_j = prev[-1]
            next_start_i, next_start_j = segment[0]
            gap_i = next_start_i - prev_end_i - 1
            gap_j = prev_end_j - next_start_j - 1
            if 0 <= gap_i <= 1 and 0 <= gap_j <= 1:
                current.append(segment)
            else:
                helix_groups.append(current)
                current = [segment]
        helix_groups.append(current)
        ...
        pair_to_stem = StemSegments.pair_map_by_tiers(ensure_pairs_list(nested_pairs))
        loop_index_by_pos = nested_loops._position_to_loop_index
        map_len = len(loop_index_by_pos)
        segment_loop_pairs: List[Tuple[int, int]] = []
        for segment in segments:
            loop_indices: Set[int] = set()
            for i, j in segment:
                if i < 0 or j < 0 or i >= map_len or j >= map_len:
                    return None
                idx_i = loop_index_by_pos[i]
                idx_j = loop_index_by_pos[j]
        ...
        loop1 = nested_loops[idx1]
        loop2 = nested_loops[idx2]
        if loop1.kind != LoopType.HAIRPIN or loop2.kind != LoopType.HAIRPIN:
            return None
        anchor_stems_1 = {pair_to_stem[pair] for pair in loop1.anchor_pairs if pair in pair_to_stem}
        anchor_stems_2 = {pair_to_stem[pair] for pair in loop2.anchor_pairs if pair in pair_to_stem}
        if len(anchor_stems_1) != 1 or len(anchor_stems_2) != 1:
            return None
        helix_count = sum(1 for group in helix_groups if sum(len(segment) for segment in group) >= 2)
        if helix_count == 0:
            return None
        return nested_loops, (idx1, idx2), helix_count

    @classmethod
    def _kissing_hairpin_loops(cls, pairs: List[Pair], length: int) -> Loops | None:
        kissing = cls._kissing_hairpin_signature(pairs, length)
        if kissing is None:
            return None
        nested_loops, loop_pair, helix_count = kissing
        if helix_count != 1:
            return None
...
class LoopSegments:
    ...
    def _segments_for_kind(self, kind: LoopSegmentType, *, side: EndSide | None = None) -> List[LoopSegment]:
        segments = [segment for segment in self._segments_cache if segment.kind == kind]
        if side is None:
            return segments
        return [segment for segment in segments if segment.side == side]
    ...
    @cached_property
    def hairpins(self) -> List[LoopSegment]:
        return self._segments_for_kind(LoopSegmentType.HAIRPIN)

    @cached_property
    def bulge_loops(self) -> List[LoopSegment]:
        return self._segments_for_kind(LoopSegmentType.BULGE)

    @cached_property
    def internal_loops(self) -> List[LoopSegment]:
        return self._segments_for_kind(LoopSegmentType.INTERNAL)

    @cached_property
    def branches(self) -> List[LoopSegment]:
        return self._segments_for_kind(LoopSegmentType.BRANCH)

    @cached_property
    def external_loops(self) -> List[LoopSegment]:
        return self._segments_for_kind(LoopSegmentType.EXTERNAL)

    @cached_property
    def end_5p(self) -> LoopSegment | None:
        return self._end_segment(EndSide.FIVE_PRIME)

    @cached_property
    def end_3p(self) -> LoopSegment | None:
        return self._end_segment(EndSide.THREE_PRIME)

    @property
    def ends(self) -> Tuple[LoopSegment | None, LoopSegment | None]:
        return (self.end_5p, self.end_3p)

    @cached_property
    def all_segments(self) -> List[LoopSegment]:
        return list(self._segments_cache)

    def _end_segment(self, side: EndSide) -> LoopSegment | None:
        segments = self._segments_for_kind(LoopSegmentType.END)
        for segment in segments:
