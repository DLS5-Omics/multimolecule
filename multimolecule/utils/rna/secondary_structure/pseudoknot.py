...
from .pairs import (
    ...
    segment_list_to_pairs,
)
from .types import Pair, Pairs, PairsList, Segment, Tiers
...
def _torch_split_pseudoknot_pairs(pairs: Tensor) -> Tuple[Tensor, Tensor]:
    ...
    if pairs.shape[0] < 2:
        return pairs, empty
    device = pairs.device
    segments = pairs_to_duplex_segment_list(pairs)
    if not segments:
        return pairs, empty
    start_i = torch.tensor([seg[0][0] for seg in segments], dtype=torch.long, device=device)
    start_j = torch.tensor([seg[0][1] for seg in segments], dtype=torch.long, device=device)
    lengths = torch.tensor([len(seg) for seg in segments], dtype=torch.long, device=device)
    nested_idx = _torch_mwis_segments(start_i, start_j, lengths)
    if nested_idx.numel() == 0:
        return empty, pairs

    nested_idx_list = [int(idx) for idx in nested_idx.tolist()]
    nested_idx_set = set(nested_idx_list)
    nested_segments = [segments[idx] for idx in nested_idx_list]
    knot_segments = [seg for idx, seg in enumerate(segments) if idx not in nested_idx_set]
    primary = _torch_pairs_from_segments(nested_segments, device)
    pseudoknot = _torch_pairs_from_segments(knot_segments, device)
    return primary, pseudoknot


def _torch_pairs_from_segments(segments: List[Segment], device: torch.device) -> Tensor:
    if not segments:
        return torch.empty((0, 2), dtype=torch.long, device=device)
    pairs = [pair for seg in segments for pair in seg]
    if not pairs:
        return torch.empty((0, 2), dtype=torch.long, device=device)
...
def _numpy_split_pseudoknot_pairs(pairs: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if not segments:
        return pairs, empty
    nested_idx = _numpy_mwis_segments(segments)
    if nested_idx.size == 0:
        return empty, pairs
    nested_set = set(nested_idx.tolist())
    nested_segments = [segments[idx] for idx in nested_idx.tolist()]
    knot_segments = [seg for idx, seg in enumerate(segments) if idx not in nested_set]
    primary = segment_list_to_pairs(nested_segments, empty)
    pseudoknot = segment_list_to_pairs(knot_segments, empty)
    return primary, pseudoknot


def _numpy_mwis_segments(segments: List[Segment]) -> np.ndarray:
    if not segments:
        return np.empty((0,), dtype=int)

    start_i = np.array([seg[0][0] for seg in segments], dtype=int)
    start_j = np.array([seg[0][1] for seg in segments], dtype=int)
    lengths = np.array([len(seg) for seg in segments], dtype=int)
...
def _torch_crossing_pairs(pairs: Tensor) -> Tensor:
    ...
    if len(segments) < 2:
        return pairs.new_empty((0, 2))
    outer_pairs = torch.tensor([seg[0] for seg in segments], dtype=torch.long, device=pairs.device)
    if outer_pairs.shape[0] < 2:
        return pairs.new_empty((0, 2))
    seg_mask = _torch_crossing_mask(outer_pairs)
    if not bool(seg_mask.any().item()):
        return pairs.new_empty((0, 2))
    crossing_segments = [seg for seg, keep in zip(segments, seg_mask.tolist()) if keep]
    crossing_pairs = _torch_pairs_from_segments(crossing_segments, pairs.device)
...
def _numpy_crossing_pairs(pairs: np.ndarray) -> np.ndarray:
    ...
    if len(segments) < 2:
        return empty
    outer_pairs = np.array([seg[0] for seg in segments], dtype=int)
    if outer_pairs.size == 0:
        return empty
    seg_mask = _numpy_crossing_mask(outer_pairs)
    if not np.any(seg_mask):
        return empty
    crossing_segments = [seg for seg, keep in zip(segments, seg_mask) if keep]
    return segment_list_to_pairs(crossing_segments, empty)
