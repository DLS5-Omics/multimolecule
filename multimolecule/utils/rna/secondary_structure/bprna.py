...
class BpRnaSecondaryStructureTopology:
    ...
    def structural_annotation(self) -> str:
        ...
        segments = loop_segments.all_segments
        if segments:
    ...
    def structure_types(self) -> Dict[str, List[str]]:
        ...
        for loop_segment in loop_segments.bulge_loops:
            knots_in = knots_for_span(loop_segment.start, loop_segment.stop)
            bulge_loops.append((loop_segment.start, loop_segment.stop, knots_in))
        for loop_segment in loop_segments.internal_loops:
            internal_spans.add((loop_segment.start, loop_segment.stop))
        for loop_segment in loop_segments.external_loops:
            knots_in = knots_for_span(loop_segment.start, loop_segment.stop)
            external_spans.append((loop_segment.start, loop_segment.stop, knots_in))
        for loop_segment in loop_segments.all_segments:
            if loop_segment.kind != LoopSegmentType.END:
                continue
            knots_in = knots_for_span(loop_segment.start, loop_segment.stop)
        ...
        multiloops.sort(key=lambda spans: spans[0][0] if spans else -1)
        for m, spans in enumerate(multiloops, start=1):
            spans.sort(key=lambda span: span[0])
