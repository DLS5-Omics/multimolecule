...
def _build_datasets(root: str | Path) -> dict[str, dict[str, list[dict]]]:
    root = Path(root)
    bprna_1m = [convert_bpseq(file) for file in tqdm(_get_files(root / "from_bpRNA-1m", ".bpseq"), desc="from_bpRNA-1m")]
    rfam_14_5 = [
    ...
    try:
        for suffix, data in datasets.items():
            ...
            convert_config.repo_id = (
                f"{base_repo_id}-{suffix}" if suffix and base_repo_id else base_repo_id
            )
