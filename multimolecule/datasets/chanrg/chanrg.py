...
def load_metadata():
    ...
    return clans, architectures, super_families


def convert_family(family, metadata=None):
    if metadata is None:
        metadata = load_metadata()
    clans, architectures, super_families = metadata
...
def convert_dataset(convert_config):
    ...
    data = [
        ...
        for i in convert_family(os.path.join(convert_config.dataset_path, family), metadata)
    ]
    data = pd.DataFrame(data)
    unique_clans = sorted([i for i in data["clan"].unique() if i is not None])
