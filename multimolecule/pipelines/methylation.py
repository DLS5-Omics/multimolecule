...
def _resolve_sequence(inputs: str | Mapping[str, Any]) -> str:
    if isinstance(inputs, Mapping):
        ...
        sequence = inputs
    if not sequence:
        raise ValueError("methylation requires a DNA sequence.")
    return str(sequence)
...
def _output_channels(model, scores: Tensor) -> list[str]:
    ...
    return [f"methylation_{index}" for index in range(size)]
...
def _methylation_result(sequence: str, scores: Tensor, channels: list[str]) -> dict[str, Any]:
    values = scores.reshape(-1).tolist()
    result: dict[str, Any] = {"sequence": sequence, "channels": channels[: len(values)]}
    if len(values) == 1:
