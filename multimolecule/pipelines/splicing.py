...
def _processed_scores(model_outputs: Any, *, model: Any, config: Any, task: str) -> tuple[Tensor, list[str]]:
    channels = None
    ...
    if scores.ndim == 1:
        return scores, list(channels) if channels is not None else _sequence_channels(scores.shape[-1], config)

    num_channels = scores.shape[-1]
    if channels is not None:
        return scores, list(channels)
    if num_channels == 3:
        return scores.softmax(dim=-1), _splice_channels()
    if num_channels == 2:
        return scores.softmax(dim=-1), ["no_splice", "splice_site"]
    return scores, _sequence_channels(num_channels, config)


def _sample_tensors(scores: Tensor, sample_count: int) -> list[Tensor]:
    if sample_count == 1 and scores.ndim <= 2:
        return [scores]
    return list(scores)
...
__all__ = ["SpliceSitePipeline", "SpliceVariantEffectPipeline"]
