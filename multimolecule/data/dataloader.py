...
class DataLoader(data.DataLoader):
    def __init__(
        ...
        collate_fn: Callable[..., Any] = no_collate,
        **kwargs: Any,
    ) -> None:
        is_train = None
        ...
        sampler: data.Sampler[Any] = (
            data.distributed.DistributedSampler(dataset, shuffle=shuffle)
            if distributed
            else data.RandomSampler(sized_dataset) if shuffle else data.SequentialSampler(sized_dataset)
        )
    ...
    def __repr__(self) -> str:
        return (
            f"{self.__class__.__name__}(dataset={self.dataset}, batch_size={self._batch_size}, "
            f"shuffle={self._shuffle}, drop_last={self._drop_last}, num_workers={self.num_workers}, "
