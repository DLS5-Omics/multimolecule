...
class SpliceBertForMaskedLM(SpliceBertPreTrainedModel):
    ...
    _tied_weights_keys = {
        "lm_head.decoder.weight": "splicebert.embeddings.word_embeddings.weight",
...
class SpliceBertForPreTraining(SpliceBertForMaskedLM):
    pass
...
class SpliceBertEncoder(nn.Module):
    def __init__(self, config: SpliceBertConfig):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([SpliceBertLayer(config, layer_idx=i) for i in range(config.num_hidden_layers)])
...
class SpliceBertLayer(GradientCheckpointingLayer):
    def __init__(self, config: SpliceBertConfig, layer_idx: int | None = None):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = SpliceBertAttention(config, layer_idx=layer_idx, is_causal=config.is_decoder)
...
class SpliceBertAttention(nn.Module):
    def __init__(
        ...
        attention_class = SpliceBertCrossAttention if is_cross_attention else SpliceBertSelfAttention
...
class SpliceBertCrossAttention(nn.Module):
    def __init__(
        self,
        config: SpliceBertConfig,
        position_embedding_type: str | None = None,
...
SpliceBertPreTrainedModel._can_record_outputs = {
    "hidden_states": SpliceBertLayer,
    "attentions": OutputRecorder(SpliceBertAttention, index=1, layer_name="attention"),
    "cross_attentions": OutputRecorder(SpliceBertAttention, index=1, layer_name="crossattention"),
}
