...
class DnaBert2PreTrainedModel(PreTrainedModel):
    """
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, DnaBert2Encoder):
...
class DnaBert2Model(DnaBert2PreTrainedModel):
    ...
    Examples:
        >>> import torch
        >>> from multimolecule import DnaBert2Config, DnaBert2Model, RnaTokenizer
    ...
    def __init__(self, config: DnaBert2Config, add_pooling_layer: bool = True):
        super().__init__(config)
        self.pad_token_id = config.pad_token_id
        self.embeddings = DnaBert2Embeddings(config)
        self.encoder = DnaBert2Encoder(config)
        self.pooler = DnaBert2Pooler(config) if add_pooling_layer else None
...
class DnaBert2ForSequencePrediction(DnaBert2PreTrainedModel):
    ...
    def __init__(self, config: DnaBert2Config):
        super().__init__(config)
        self.dnabert2 = DnaBert2Model(config)
...
class DnaBert2ForTokenPrediction(DnaBert2PreTrainedModel):
    ...
    def __init__(self, config: DnaBert2Config):
        super().__init__(config)
        self.dnabert2 = DnaBert2Model(config, add_pooling_layer=False)
...
class DnaBert2ForMaskedLM(DnaBert2PreTrainedModel):
    ...
    def __init__(self, config: DnaBert2Config):
        ...
        if config.is_decoder:
            warn(
                "If you want to use `DnaBert2ForMaskedLM` make sure `config.is_decoder=False` for "
...
class DnaBert2Embeddings(nn.Module):
    ...
    def forward(
        ...
        # DNA models do not use token_type_ids
...
class DnaBert2Encoder(nn.Module):
    def __init__(self, config: DnaBert2Config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([DnaBert2Layer(config) for _ in range(config.num_hidden_layers)])
        ...
        if self.config.position_embedding_type == "alibi":
            self._current_alibi_size = int(512)
            self.alibi = torch.zeros((1, self.num_attention_heads, self._current_alibi_size, self._current_alibi_size))
            self.rebuild_alibi_tensor(size=512)

    def rebuild_alibi_tensor(self, size: int, device: torch.device | str | None = None):
        # Alibi
...
class DnaBert2Attention(nn.Module):
    def __init__(self, config: DnaBert2Config, position_embedding_type: str | None = None):
        super().__init__()
        self.self = DnaBert2SelfAttention(config, position_embedding_type=position_embedding_type)
