...
class SpotRnaModule(nn.Module):
    ...
    def __init__(self, config: SpotRnaConfig, module_config: SpotRnaModuleConfig):
        super().__init__()
        self.gradient_checkpointing = False

        # Some checkpoints use per-member input stats on top of the shared normalization.
        ...
        self.projection = nn.Conv2d(config.input_channels, module_config.conv_channels, kernel_size=3, padding=1)
        self.layers = nn.ModuleList(
            [
                SpotRnaConvBlock(config, module_config, block_index=block_index)
                for block_index in range(module_config.num_conv_blocks)
            ]
        )
        self.activation = ACT2FN[module_config.output_act]
        self.layer_norm = SpotRnaLayerNorm(module_config.conv_channels)

        if module_config.num_blstm_blocks > 0:
            self.bilstm = SpotRna2DBiLSTM(module_config)
            classifier_input_size = module_config.blstm_hidden_size * 4
        else:
            self.bilstm = None
            classifier_input_size = module_config.conv_channels

        classifier = []
        for block_index in range(module_config.num_fc_blocks):
            input_size = classifier_input_size if block_index == 0 else module_config.fc_hidden_size
            classifier.append(SpotRnaFCBlock(config, module_config, input_size, module_config.fc_hidden_size))
        self.classifier = nn.ModuleList(classifier)

        output_size = module_config.fc_hidden_size if module_config.num_fc_blocks > 0 else classifier_input_size
    ...
    def forward(self, hidden_state: Tensor) -> Tensor:
        ...
        for layer in self.layers:
            if self.gradient_checkpointing and self.training:
                hidden_state = self._gradient_checkpointing_func(layer.__call__, hidden_state, valid_pair_mask)
            else:
        ...
        if self.bilstm is not None:
            ...
            hidden_state = hidden_state.permute(0, 3, 1, 2)
            if self.gradient_checkpointing and self.training:
                hidden_state = self._gradient_checkpointing_func(self.bilstm.__call__, hidden_state)
            else:
...
class SpotRnaConvBlock(nn.Module):
    def __init__(self, config: SpotRnaConfig, module_config: SpotRnaModuleConfig, block_index: int = 0):
        super().__init__()
        ...
        dilation1 = 2 ** (conv3_idx % module_config.dilation_cycle) if module_config.use_dilation else 1
        dilation2 = 2 ** (conv5_idx % module_config.dilation_cycle) if module_config.use_dilation else 1

        self.act1 = ACT2FN[module_config.hidden_act]
        ...
        self.act2 = ACT2FN[module_config.hidden_act]
...
class SpotRna2DBiLSTM(nn.LSTM):
    def __init__(self, module_config: SpotRnaModuleConfig):
        super().__init__(
            input_size=module_config.conv_channels,
    ...
    def _forward_packed(self, sequence: Tensor, sequence_length: int, batch_size: int, hx=None) -> Tensor:
        ...
        if output.dtype != valid_output.dtype:
            output = output.to(valid_output.dtype)
        output[valid_sequences] = valid_output
        return output


class SpotRnaFCBlock(nn.Module):
    def __init__(self, config: SpotRnaConfig, module_config: SpotRnaModuleConfig, in_features: int, out_features: int):
        super().__init__()
        self.dense = nn.Linear(in_features, out_features)
        hidden_act = getattr(module_config, "fc_act", None) or module_config.hidden_act
