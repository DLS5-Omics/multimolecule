...
_CHANNEL_PERMUTATION = [0, 2, 3, 1, 4, 6, 7, 5]
...
def convert_checkpoint(convert_config) -> None:
    ...
    state_dict = {}
    state_dict["input_mean"] = base_mean = model.input_mean[:, :, :, _CHANNEL_PERMUTATION]
    state_dict["input_std"] = base_std = model.input_std[:, :, :, _CHANNEL_PERMUTATION]

    for network_index, network_config in enumerate(config.networks):
        checkpoint_path = os.path.join(convert_config.checkpoint_path, f"model{network_index}")
        reader = tf.train.load_checkpoint(checkpoint_path)
        network_state = _convert_network_state_dict(network_config, reader)
        network_mean, network_std = _load_input_stats(tf, checkpoint_path)
        network_mean = network_mean[:, :, :, _CHANNEL_PERMUTATION]
        network_std = network_std[:, :, :, _CHANNEL_PERMUTATION]
        # Keep the shared preprocessing and fold checkpoint-specific input stats into an affine correction.
        network_state["input_scale"] = base_std / network_std
        network_state["input_bias"] = (base_mean - network_mean) / network_std
        for key, value in network_state.items():
            state_dict[f"networks.{network_index}.{key}"] = value
...
def _convert_network_state_dict(network_config, reader):
    state_dict = {}

    projection_weight = _convert_conv2d_kernel(reader.get_tensor("initconv/conv2d/kernel"))
    state_dict["projection.weight"] = projection_weight[:, _CHANNEL_PERMUTATION, :, :]
