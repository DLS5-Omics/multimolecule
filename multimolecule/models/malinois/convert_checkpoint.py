...
def _load_original_state_dict(checkpoint_path: str) -> OrderedDict[str, torch.Tensor]:
    def load_malinois_checkpoint(file) -> object:
    ...
    if tarfile.is_tarfile(checkpoint_path):
        with tarfile.open(checkpoint_path) as tar:
            ...
            checkpoint = load_malinois_checkpoint(io.BytesIO(file.read()))
    else:
        checkpoint = load_malinois_checkpoint(checkpoint_path)
    state_dict = (
    ...
    if not isinstance(state_dict, Mapping):
        raise TypeError(f"Expected Malinois checkpoint state dict to be a mapping, got {type(state_dict).__name__}.")
    return OrderedDict(cast(Mapping[str, torch.Tensor], state_dict))
