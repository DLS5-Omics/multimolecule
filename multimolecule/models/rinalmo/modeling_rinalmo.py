...
class RiNALMoEncoder(nn.Module):
    def __init__(self, config: RiNALMoConfig):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([RiNALMoLayer(config) for _ in range(config.num_hidden_layers)])
...
class RiNALMoLayer(nn.Module):
    def __init__(self, config: RiNALMoConfig):
        ...
        self.seq_len_dim = 1
        self.attention = RiNALMoAttention(config)
...
class RiNALMoAttention(nn.Module):
    ...
    def forward(
        ...
        attention_output = self.output(self_outputs[0], hidden_states_ln)
...
class RiNALMoSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in RiNALMoModel forward() function)
