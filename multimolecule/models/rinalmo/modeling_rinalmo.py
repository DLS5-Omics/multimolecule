...
class RiNALMoEncoder(nn.Module):
    def __init__(self, config: RiNALMoConfig):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([RiNALMoLayer(config, layer_idx=i) for i in range(config.num_hidden_layers)])
...
class RiNALMoLayer(GradientCheckpointingLayer):
    def __init__(self, config: RiNALMoConfig, layer_idx: int | None = None):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = RiNALMoAttention(config, layer_idx=layer_idx, is_causal=config.is_decoder)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = RiNALMoAttention(
                config,
...
class RiNALMoAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        attention_output = self.output(attention_output, hidden_states_ln)
        return attention_output, attn_weights


class RiNALMoSelfAttention(nn.Module):
    def __init__(
        self,
        config: RiNALMoConfig,
        position_embedding_type: str | None = None,
...
class RiNALMoCrossAttention(nn.Module):
    def __init__(
        self,
        config: RiNALMoConfig,
        position_embedding_type: str | None = None,
...
class RiNALMoSelfOutput(nn.Module):
    def __init__(self, config: RiNALMoConfig):
        super().__init__()
...
class RiNALMoIntermediate(nn.Module):
    def __init__(self, config: RiNALMoConfig):
        super().__init__()
        self.linear = nn.Linear(config.hidden_size, int(2 / 3 * config.intermediate_size))
        self.linear_gate = nn.Linear(config.hidden_size, int(2 / 3 * config.intermediate_size))
        self.bias = nn.Parameter(torch.ones(1), requires_grad=config.learnable_beta)

    def forward(self, x):
        linear_out = self.linear(x)
        swish_out = linear_out * torch.sigmoid(self.bias * linear_out)
        return swish_out * self.linear_gate(x)


class RiNALMoOutput(nn.Module):
    def __init__(self, config: RiNALMoConfig):
        super().__init__()
        self.dropout = nn.Dropout(config.hidden_dropout)
        self.dense = nn.Linear(int(2 / 3 * config.intermediate_size), config.hidden_size)
...
RiNALMoPreTrainedModel._can_record_outputs = {
    "hidden_states": RiNALMoLayer,
    "attentions": OutputRecorder(RiNALMoAttention, index=1, layer_name="attention"),
    "cross_attentions": OutputRecorder(RiNALMoAttention, index=1, layer_name="crossattention"),
}
