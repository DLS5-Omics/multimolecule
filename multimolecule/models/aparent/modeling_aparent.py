...
class AparentEncoder(nn.Module):
    ...
    def __init__(self, config: AparentConfig):
        ...
        self.conv1 = nn.Conv1d(config.vocab_size, config.conv1_filters, config.conv1_kernel_size)
        self.pool = nn.MaxPool1d(config.pool_size)
        self.conv2 = nn.Conv1d(config.conv1_filters, config.conv2_filters, config.conv2_kernel_size)
        ...
        flatten_size = conv2_out * config.conv2_filters + 1

        self.dense1 = nn.Linear(flatten_size, config.hidden_sizes[0])
    ...
    def forward(self, hidden_state: Tensor) -> Tensor:
        ...
        # deterministic constant rebuilt here rather than stored in the checkpoint.
