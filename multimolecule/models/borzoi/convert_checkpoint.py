...
def _keras_attention_bias_to_torch(weight: torch.Tensor) -> torch.Tensor:
    weight = weight.squeeze()
    if weight.ndim != 2:
        raise ValueError(f"Expected Borzoi attention bias to squeeze to 2D, got {tuple(weight.shape)}")
    return weight.unsqueeze(0).unsqueeze(2).contiguous()
...
def _add_attention(state_dict: OrderedDict[str, torch.Tensor], weights, index: int) -> None:
    ...
    # Upstream stores these as (num_heads, dim_key); the parameters are (1, num_heads, 1, dim_key).
    state_dict[f"{prefix}.rel_content_bias"] = _keras_attention_bias_to_torch(_read_mha(weights, layer, "r_w_bias:0"))
    state_dict[f"{prefix}.rel_pos_bias"] = _keras_attention_bias_to_torch(_read_mha(weights, layer, "r_r_bias:0"))
...
class ConvertConfig(ConvertConfig_):
    ...
    def post(self):
        if should_derive_output_path(self, Config.model_type):
            append_output_suffix(self, self.species)
        super().post()
