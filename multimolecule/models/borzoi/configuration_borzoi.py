...
class BorzoiConfig(PreTrainedConfig):
    ...
    Args:
        ...
        species:
            Species track set to expose downstream. Borzoi is trained with human and mouse output heads, and the
            released checkpoints are selected by species; the selected track set determines `num_labels`.
            Use `human` (7611 tracks) or `mouse` (2608 tracks).
        target_length:
    ...
    Examples:
        ...
        >>> # Initializing a Borzoi human-track style configuration
        >>> configuration = BorzoiConfig()
        >>> # Initializing a model (with random weights) from the human-track style configuration
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.num_rel_pos_features % 2 != 0:
            raise ValueError(
                f"num_rel_pos_features must be even (the positional encoding uses a signed-copy "
                f"doubling that requires divisibility by 2), got {self.num_rel_pos_features}"
            )
