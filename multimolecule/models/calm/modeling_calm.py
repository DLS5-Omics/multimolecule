...
class CaLmEncoder(nn.Module):
    def __init__(self, config: CaLmConfig):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([CaLmLayer(config, layer_idx=i) for i in range(config.num_hidden_layers)])
...
class CaLmLayer(GradientCheckpointingLayer):
    def __init__(self, config: CaLmConfig, layer_idx: int | None = None):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = CaLmAttention(config, layer_idx=layer_idx, is_causal=config.is_decoder)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = CaLmAttention(
                config,
...
class CaLmAttention(nn.Module):
    def __init__(
        ...
        attention_class = CaLmCrossAttention if is_cross_attention else CaLmSelfAttention
...
class CaLmCrossAttention(nn.Module):
    def __init__(
        self,
        config: CaLmConfig,
        position_embedding_type: str | None = None,
...
CaLmPreTrainedModel._can_record_outputs = {
    "hidden_states": CaLmLayer,
    "attentions": OutputRecorder(CaLmAttention, index=1, layer_name="attention"),
    "cross_attentions": OutputRecorder(CaLmAttention, index=1, layer_name="crossattention"),
}
