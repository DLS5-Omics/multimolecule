...
def convert_checkpoint(convert_config):
    ...
    # The Keras Flatten layer iterates over ``(sequence_length, conv_channels)`` channels-last; the
    # torch Conv1d output is ``(conv_channels, sequence_length)`` and the encoder transposes back to
...
def _convert_checkpoint(config: Config, file: str) -> OrderedDict:
    ...
    try:
        ...
        raise ImportError(
            "h5py is required to convert an Optimus 5-Prime checkpoint. " "Install it with: pip install h5py"
        ) from e
