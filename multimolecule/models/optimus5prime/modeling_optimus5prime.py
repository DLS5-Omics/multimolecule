...
class Optimus5PrimeEncoder(nn.Module):
    ...
    def __init__(self, config: Optimus5PrimeConfig):
        super().__init__()
        self.act = nn.ReLU() if config.hidden_act == "relu" else _ACT2MODULE(config.hidden_act)
        ...
        for _ in range(config.num_conv_layers):
            convs.append(nn.Conv1d(in_channels, config.conv_filters, config.conv_kernel_size))
        ...
        flatten_size = config.sequence_length * config.conv_filters
        self.dense = nn.Linear(flatten_size, config.hidden_size)
...
def _ACT2MODULE(name: str) -> nn.Module:
    # Local fallback so the encoder can use exotic ACT names without dragging ACT2FN at import time.
