...
from multimolecule.models import DnaBertSConfig as Config
from multimolecule.models import DnaBertSModel as Model
...
def convert_checkpoint(convert_config):
    print(f"Converting DNABERT-S checkpoint at {convert_config.checkpoint_path}")
    ...
    # Remove fields from the original config that are not part of DnaBertSConfig
...
def _convert_checkpoint(config, original_state_dict):
    ...
    for key, value in original_state_dict.items():
        ...
        # DNABERT-S checkpoint keys have no "bert." prefix
        match = re.match(r"encoder\.layer\.(\d+)\.attention\.self\.Wqkv\.(weight|bias)", key)
        if match:
            layer_idx = match.group(1)
            param_type = match.group(2)
            prefix = f"encoder.layer.{layer_idx}.attention.self"
    ...
    # Remove pooler weights (not used in our model by default during conversion)
