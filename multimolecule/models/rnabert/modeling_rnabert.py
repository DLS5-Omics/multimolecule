...
class RnaBertEncoder(nn.Module):
    def __init__(self, config: RnaBertConfig):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([RnaBertLayer(config, layer_idx=i) for i in range(config.num_hidden_layers)])
...
class RnaBertLayer(GradientCheckpointingLayer):
    def __init__(self, config: RnaBertConfig, layer_idx: int | None = None):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = RnaBertAttention(config, layer_idx=layer_idx, is_causal=config.is_decoder)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = RnaBertAttention(
                config,
...
class RnaBertAttention(nn.Module):
    def __init__(
        ...
        attention_class = RnaBertCrossAttention if is_cross_attention else RnaBertSelfAttention
...
class RnaBertCrossAttention(nn.Module):
    def __init__(
        self,
        config: RnaBertConfig,
        position_embedding_type: str | None = None,
