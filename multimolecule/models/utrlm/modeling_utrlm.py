...
class UtrLmPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: nn.Module):
        ...
        if isinstance(module, UtrLmEmbeddings):
            init.copy_(
                module.position_ids,
...
class UtrLmEncoder(nn.Module):
    def __init__(self, config: UtrLmConfig):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([UtrLmLayer(config, layer_idx=i) for i in range(config.num_hidden_layers)])
...
class UtrLmLayer(GradientCheckpointingLayer):
    def __init__(self, config: UtrLmConfig, layer_idx: int | None = None):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = UtrLmAttention(config, layer_idx=layer_idx, is_causal=config.is_decoder)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = UtrLmAttention(
                config,
...
class UtrLmCrossAttention(nn.Module):
    def __init__(
        self,
        config: UtrLmConfig,
        position_embedding_type: str | None = None,
...
UtrLmPreTrainedModel._can_record_outputs = {
    "hidden_states": UtrLmLayer,
    "attentions": OutputRecorder(UtrLmAttention, index=1, layer_name="attention"),
    "cross_attentions": OutputRecorder(UtrLmAttention, index=1, layer_name="crossattention"),
}
