...
from multimolecule.models.conversion_utils import convert_one_hot_embeddings, save_checkpoint
...
# Upstream OpenSpliceAI one-hot input channel order (see openspliceai/predict/predict.py IN_MAP):
#   1: A; 2: C; 3: G; 4: T  ->  channels [A, C, G, T]
ORIGINAL_NUCLEOTIDE_ORDER = ["A", "C", "G", "T"]

StageKind = Literal["block", "skip"]
StageLayout = tuple[int, int, StageKind, int | None]
Variant = tuple[str, int]

DEFAULT_VARIANT = "openspliceai-mane-10000nt"
DEFAULT_SEED = 10
...
def _variant_name(species: str, context: int) -> str:
    return f"openspliceai-{species}-{context}nt"
...
def convert_original_state_dict_value(key: str, value: torch.Tensor, new_vocab: list) -> torch.Tensor:
    ...
    # First learned layer sees vocabulary-dependent channels; delegate token conversion to the DNA tokenizer rules.
