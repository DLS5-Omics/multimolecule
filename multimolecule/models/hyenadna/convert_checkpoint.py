...
original_vocab_list = ["<cls>", "<eos>", "<null>", "<mask>", "<pad>", "<null>", "<unk>", "A", "C", "G", "T", "N"]
...
def _convert_checkpoint(config, original_state_dict, original_vocab, new_vocab):
    ...
    for key, value in original_state_dict.items():
        ...
        if new_key.endswith((".filter_fn.pos_emb.z", ".filter_fn.pos_emb.t", ".filter_fn.modulation.deltas")):
            continue
        if not config.train_freq and ".filter_fn.implicit_filter." in new_key and new_key.endswith(".freq"):
            continue
...
# Map original checkpoint names to our consistent size-based naming.
# Multiple original checkpoints share the same architecture but differ only in context length.
# We use one canonical name per architecture, picking the longest context checkpoint as canonical.
_CHECKPOINT_NAME_MAP = {
    "hyenadna-large-1m-seqlen-hf": "hyenadna-large",  # (8, 256)
    "hyenadna-small-32k-seqlen-hf": "hyenadna-medium",  # (4, 256)
    "hyenadna-tiny-1k-seqlen-d256-hf": "hyenadna-small",  # (2, 256)
    "hyenadna-tiny-16k-seqlen-d128-hf": "hyenadna-tiny",  # (2, 128)
}
...
class ConvertConfig(ConvertConfig_):
    ...
    default_variant: str | None = "hyenadna-large"

    def post(self):
        if self.output_path is None:
            basename = os.path.basename(self.checkpoint_path.rstrip("/")).lower()
            self.output_path = _CHECKPOINT_NAME_MAP.get(basename, "-".join(basename.split("-")[:2]))
