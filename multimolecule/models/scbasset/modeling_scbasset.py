...
class ScBassetEncoder(nn.Module):
    def __init__(self, config: ScBassetConfig):
        ...
        self.bottleneck = ScBassetBottleneck(config, conv_output_size)
        self.activation = ACT2FN[config.hidden_act]
        self.gradient_checkpointing = False

    @staticmethod
    def _conv_output_size(config: ScBassetConfig) -> int:
...
class ScBassetBottleneck(nn.Module):
    ...
    def __init__(self, config: ScBassetConfig, conv_output_size: int):
        super().__init__()
        self.dense = nn.Linear(conv_output_size, config.bottleneck_size, bias=False)
...
@dataclass
class ScBassetModelOutput(ModelOutput):
    """
    Base class for outputs of the scBasset backbone.

    Args:
        last_hidden_state (`torch.FloatTensor` of shape `(batch_size, hidden_size)`):
            Final bottleneck embedding produced by the scBasset encoder.
        pooler_output (`torch.FloatTensor` of shape `(batch_size, hidden_size)`):
            Same tensor as `last_hidden_state`; scBasset collapses the sequence dimension in its encoder.
        hidden_states (`tuple(torch.FloatTensor)`, *optional*, returned when `output_hidden_states=True` is passed or
            when `config.output_hidden_states=True`):
            Tuple containing the one-hot embedding output and the final bottleneck embedding.
        attentions:
            Always `None`; scBasset is a convolutional model and has no attention layers. Provided for compatibility
...
ScBassetPreTrainedModel._can_record_outputs = {"hidden_states": ScBassetEncoder}
