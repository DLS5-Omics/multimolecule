...
class RibonanzaNetForPreTraining(RibonanzaNetPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        losses = (loss_2a3, loss_dms)
        loss = sum(l for l in losses if l is not None) if any(losses) else None  # noqa: E741
...
class RibonanzaNetForSecondaryStructurePrediction(RibonanzaNetForPreTraining):
    ...
    def forward(  # type: ignore[override]
        ...
        **kwargs,
        ...
        logits_2a3, loss_2a3 = output_2a3.logits, output_2a3.loss
        logits_dms, loss_dms = output_dms.logits, output_dms.loss
        losses = (loss_2a3, loss_dms, loss_ss)
        loss = sum(l for l in losses if l is not None) if any(losses) else None  # noqa: E741
...
class RibonanzaNetEncoder(nn.Module):
    ...
    def forward(
    ...
    ) -> Tuple[Tensor, ...] | RibonanzaNetModelOutput:
        all_hidden_states = () if output_hidden_states else None
        all_pairwise_states = () if output_pairwise_states else None
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if attention_mask is not None:
                if not self.fix_attention_mask:
                    attention_mask = (attention_mask_ == 1).float() if i == 0 else attention_mask_.clone()
...
class RibonanzaNetSelfOutput(nn.Module):
    def __init__(self, config: RibonanzaNetConfig):
        ...
        self.fix_attention_norm = config.fix_attention_norm
        self.layer_norm2 = None
        if not self.fix_attention_norm:
...
class RibonanzaNetPairwiseIntermediate(nn.Module):
    def __init__(self, config: RibonanzaNetConfig):
        ...
        if isinstance(config.hidden_act, str):
            self.intermediate_act_fn = ACT2FN[config.pairwise_hidden_act]
        else:
            self.intermediate_act_fn = config.hidden_act
...
class RibonanzaNetSecondaryStructurePredictionHead(BasePredictionHead):
    ...
    def __init__(self, config: RibonanzaNetConfig, head_config: HeadConfig | None = None):
        ...
        self.dense = nn.Linear(config.pairwise_size, self.config.num_labels)
    ...
    def forward(  # type: ignore[override]  # pylint: disable=arguments-renamed
        ...
        pairwise_state = pairwise_state * attention_mask[..., None]

        # remove cls token pairwise_state
        if self.bos_token_id is not None:
            pairwise_state = pairwise_state[..., 1:, 1:, :]
            attention_mask = attention_mask[..., 1:, 1:]
            if input_ids is not None:
                input_ids = input_ids[..., 1:]
        # remove eos token pairwise_state
        if self.eos_token_id is not None:
            ...
            pairwise_state = pairwise_state * eos_mask[..., None]
            pairwise_state = pairwise_state[..., :-1, :-1, :]
            attention_mask = attention_mask[..., :-1, :-1]
