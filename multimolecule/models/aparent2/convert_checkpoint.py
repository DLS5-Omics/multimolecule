...
# "N" as 0.25 in each nucleotide channel. MultiMolecule exposes 3'UTR/polyA sequence as RNA, so the upstream
# T channel is mapped to U and the converted "N" projection channel is the mean of the four canonical channels.
...
def _h5_weights(path: str):
    ...
    Returns an ``(f, get)`` tuple where ``f`` is the open h5py.File handle (caller must close it)
    and ``get(layer, name)`` reads a single named weight tensor into a torch.Tensor.
    """
