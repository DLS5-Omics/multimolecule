...
class Aparent2Model(Aparent2PreTrainedModel):
    ...
    *input-schema* concern: score a reference and an alternate sequence separately and compare their cleavage /
    ...
    Examples:
        >>> from multimolecule import Aparent2Config, Aparent2Model, RnaTokenizer
...
class Aparent2ForSequencePrediction(Aparent2PreTrainedModel):
    ...
    Examples:
        >>> import torch
        >>> from multimolecule import Aparent2Config, Aparent2ForSequencePrediction, RnaTokenizer
        >>> config = Aparent2Config()
...
class Aparent2LibraryBias(nn.Module):
    ...
    def forward(self, sequence_score: Tensor) -> Tensor:
        ...
        # Append a trailing "no-cleavage" position initialized to 0. Rebuilt each forward
        # because the leading dimension is batch_size — cannot be a static buffer.
        ...
        # Deterministic one-hot training-sub-library indicator, rebuilt every forward pass.
        # Cannot be a static registered buffer: it must match the active dtype (e.g. fp16/bf16)
        # and device, which may change after `.to()` / `.half()` calls.
