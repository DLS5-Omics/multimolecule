...
LOGIT_CLIP = 1e-5
# Default RnaTokenizer ids reserve six special-token slots before A/C/G/U/N.
STANDARD_RNA_TOKEN_OFFSET = 6
...
class MmSpliceEmbedding(nn.Module):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            if input_ids is None:
                raise ValueError("You have to specify either input_ids or inputs_embeds")
            channel_ids = self._channel_ids(input_ids)
    ...
    def _channel_ids(self, input_ids: Tensor) -> Tensor:
        channel_ids = input_ids.to(torch.long)
        if torch.any(channel_ids >= STANDARD_RNA_TOKEN_OFFSET):
            channel_ids = channel_ids - STANDARD_RNA_TOKEN_OFFSET
        return channel_ids
...
class MmSpliceDeltaLogitPsiHead(nn.Module):
    ...
    def __init__(self):
        ...
        self.register_buffer("intercept", torch.tensor(0.0006480262366686865, dtype=torch.float32))
    ...
    def forward(self, delta_logits: Tensor) -> Tensor:
        ...
        coefficients = self.coefficients.to(device=delta_logits.device, dtype=delta_logits.dtype)
        intercept = self.intercept.to(device=delta_logits.device, dtype=delta_logits.dtype)
