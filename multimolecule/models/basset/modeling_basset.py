...
class BassetEncoder(nn.Module):
    ...
    @staticmethod
    def _flattened_size(config: BassetConfig) -> int:
        # Upstream Basset (davek44/Basset `pretrained_model.th`) uses *valid* (zero-padding)
        # convolutions followed by ceil-mode max-pooling, so the sequence is trimmed by
        # ``kernel_size - 1`` at every conv and ceil-divided at every pool.
...
class BassetConvLayer(nn.Module):
    def __init__(
        ...
        super().__init__()
        # Upstream Basset convolves with *valid* padding (``padW = 0``) and pools with
        # ceil-mode max-pooling; replicate both so the converted weights produce identical
        # activations to the original Torch7 `pretrained_model.th`.
