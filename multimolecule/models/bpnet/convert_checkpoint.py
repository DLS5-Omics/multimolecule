...
def convert_checkpoint(convert_config):
    ...
    config = Config()
    # Persist the OSKN task names so `output_channels` (and the inference widget) recover them from config.
    config.id2label = dict(enumerate(TASKS))
    config.label2id = {label: index for index, label in enumerate(TASKS)}
