...
class SpliceAiModel(SpliceAiPreTrainedModel):
    ...
    def forward(
        self, input_ids: torch.LongTensor, output_hidden_states: bool = False, return_dict: bool = True
    ) -> SpliceAiModelOutput | Tuple[Tensor, Tuple[Tensor, ...]] | Tensor:
        input_ids = F.one_hot(input_ids, num_classes=self.num_tokens)[..., : self.vocab_size]
        inputs_embeds = input_ids.transpose(1, 2).float()
        ...
        all_outputs = [
            module(inputs_embeds, output_hidden_states=output_hidden_states, return_dict=return_dict)
...
class SpliceAiModule(nn.Module):
    ...
    def __init__(self, config: SpliceAiConfig):
        super().__init__()
        self.embedding = nn.Conv1d(config.vocab_size, config.hidden_size, 1)
    ...
    def forward(
        ...
        outputs = self.encoder(embedding, output_hidden_states, return_dict)
        context = outputs.last_hidden_state if return_dict else outputs[0]
        ...
        return SpliceAiModelOutput(loss=loss, logits=logits, hidden_states=outputs.hidden_states)
...
class SpliceAiEncoder(nn.Module):
    ...
    def forward(
        self, embedding: Tensor, output_hidden_states: bool = False, return_dict: bool = True
    ) -> BaseModelOutputWithNoAttention | Tuple[Tensor, Tuple[Tensor, ...]] | Tensor:
        hidden_states = () if output_hidden_states else None

        context = self.conv(embedding)
        for stage in self.stages:
            ...
            if self.gradient_checkpointing and self.training:
                embedding, context = self._gradient_checkpointing_func(stage.__call__, embedding, context)
            else:
                embedding, context = stage(embedding, context)
        ...
        return BaseModelOutputWithNoAttention(last_hidden_state=context, hidden_states=hidden_states)
...
class SpliceAiStage(nn.Module):
    ...
    def forward(self, feature: Tensor, context: Tensor) -> Tuple[Tensor, Tensor]:
        feature = self.blocks(feature)
        context = self.conv(feature) + context
