...
class DeepCpgDnaEncoder(nn.Module):
    def __init__(self, config: DeepCpgDnaConfig):
        ...
        self.bottleneck = DeepCpgDnaBottleneck(config, self.flattened_size)
        self.activation = ACT2FN[config.hidden_act]
        self.dropout = nn.Dropout(config.hidden_dropout)
        self.gradient_checkpointing = False

    @staticmethod
    def _flattened_size(config: DeepCpgDnaConfig) -> int:
...
class DeepCpgDnaBottleneck(nn.Module):
    ...
    def __init__(self, config: DeepCpgDnaConfig, flattened_size: int):
        super().__init__()
        self.dense = nn.Linear(flattened_size, config.bottleneck_size)
