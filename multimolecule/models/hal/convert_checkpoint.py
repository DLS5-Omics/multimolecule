...
# The model index map in `modeling_hal.HalEmbedding` uses the streamline DNA order ["A", "C", "G", "T"].
# The published HAL artifact enumerates rows in the author's order ["A", "T", "C", "G"], so conversion maps
# each hexamer string into the model's base-4 feature index explicitly.
ORIGINAL_NUCLEOTIDES = ["A", "C", "G", "T"]
...
def hexamer_to_index(hexamer: str, nucleobase_size: int) -> int:
    r"""Map a hexamer string (DNA or RNA, U treated as T) to its base-`nucleobase_size` feature index."""
    index = 0
    for base in hexamer:
        base = "T" if base in ("U", "u") else base.upper()
