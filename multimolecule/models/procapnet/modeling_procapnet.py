...
class ProCapNetProfileCountHead(nn.Module):
    ...
    - The count branch global-average-pools the backbone features and applies a linear layer producing a single
      strand-merged log-count scalar of shape `(batch_size, 1)`. It is trained with mean-squared error on the
      `log(count + 1)` total.
