...
class EsmCForPreTraining(EsmCForMaskedLM):
    pass


class EsmCEmbeddings(nn.Module):
    """Token embeddings for ESMC."""
...
class EsmCEncoder(nn.Module):
    def __init__(self, config: EsmCConfig):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([EsmCLayer(config, layer_idx=i) for i in range(config.num_hidden_layers)])
        self.layer_norm = nn.LayerNorm(
            config.hidden_size,
            eps=config.layer_norm_eps,
            bias=config.final_layer_norm_bias,
        )
...
class EsmCLayer(GradientCheckpointingLayer):
    def __init__(self, config: EsmCConfig, layer_idx: int | None = None):
        ...
        self.seq_len_dim = 1
        self.attention = EsmCAttention(config, layer_idx=layer_idx)
        self.layer_norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.intermediate = EsmCIntermediate(config)
        self.output = EsmCOutput(config)
...
class EsmCAttention(nn.Module):
    def __init__(self, config: EsmCConfig, layer_idx: int | None = None):
        ...
        self.layer_norm = nn.LayerNorm(
            config.hidden_size,
            eps=config.layer_norm_eps,
            bias=config.attention_layer_norm_bias,
        )
        self.self = EsmCSelfAttention(config, layer_idx=layer_idx)
        self.output = EsmCSelfOutput(config)
...
class EsmCSelfAttention(nn.Module):
    """Multi-headed self-attention with query/key layer norm and rotary position embeddings."""
    ...
    def __init__(self, config: EsmCConfig, layer_idx: int | None = None):
        ...
        if config.qk_layer_norm:
            self.query_layer_norm = nn.LayerNorm(
                config.hidden_size,
                eps=config.layer_norm_eps,
                bias=config.qk_layer_norm_bias,
            )
            self.key_layer_norm = nn.LayerNorm(
                config.hidden_size,
                eps=config.layer_norm_eps,
                bias=config.qk_layer_norm_bias,
            )
        else:
            self.query_layer_norm = nn.Identity()
            self.key_layer_norm = nn.Identity()

        self.dropout = nn.Dropout(config.attention_dropout)
        self.rotary_embeddings = RotaryEmbedding(embedding_dim=self.head_dim)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        query_states = self.query_layer_norm(self.query(hidden_states)).view(hidden_shape).transpose(1, 2)
        key_states = self.key_layer_norm(self.key(hidden_states)).view(hidden_shape).transpose(1, 2)
...
class EsmCSelfOutput(nn.Module):
    def __init__(self, config: EsmCConfig):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size, bias=config.attention_bias)
        self.dropout = nn.Dropout(config.hidden_dropout)
        self.residue_scaling_factor = config.residue_scaling_factor

    def forward(self, hidden_states: Tensor, input_tensor: Tensor) -> Tensor:
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = hidden_states / self.residue_scaling_factor
        hidden_states = hidden_states + input_tensor
        return hidden_states


class EsmCIntermediate(nn.Module):
    """SwiGLU feed-forward block matching ESMC's packed upstream ``fc1_weight`` layout."""

    def __init__(self, config: EsmCConfig):
        super().__init__()
        self.gate_proj = nn.Linear(config.hidden_size, config.intermediate_size, bias=config.feedforward_bias)
        self.up_proj = nn.Linear(config.hidden_size, config.intermediate_size, bias=config.feedforward_bias)
...
EsmCPreTrainedModel._can_record_outputs = {
    "hidden_states": EsmCLayer,
    "attentions": OutputRecorder(EsmCAttention, index=1, layer_name="attention"),
}
