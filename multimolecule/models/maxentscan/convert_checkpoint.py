...
def convert_checkpoint(convert_config):
    ...
    # We cannot use `load_checkpoint` here because that calls `model.load_state_dict(assign=True)`,
    # which requires the dict to contain *all* model state keys — but `tables` only contains the
    # persistent score-table buffers. The hand-rolled copy_ loop is intentional.
...
class ConvertConfig(ConvertConfig_):
    ...
    def post(self):
        for mode in ("score5", "score3"):
            suffix = f"-{mode}"
            if self.output_path.endswith(suffix):
                self.output_path = self.output_path[: -len(suffix)]
                break
        self.output_path = f"{self.output_path}-{self.mode}"
        super().post()
