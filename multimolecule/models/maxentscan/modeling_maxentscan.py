...
# The streamline DNA alphabet is `ACGTN`. MaxEntScan only scores `ACGT`; the maximum-entropy
# tables are indexed in base-4 with the order A=0, C=1, G=2, T=3, which matches the streamline
# token order, so token ids are used directly as base-4 digits. This is exactly the
# `tr/ACGT/0123/` mapping used by the original Yeo & Burge (2004) `score5.pl` / `score3.pl`.
...
class MaxEntScanScorer(nn.Module):
    ...
    * ``score3``: the maximum-entropy decomposition ``sc0*sc1*sc2*sc3*sc4 / (sc5*sc6*sc7*sc8)``
    ...
    def forward(self, input_ids: Tensor) -> Tensor:
        ...
        # MaxEntScan only models ACGT; clamp the `N`/unknown token onto A so lookups stay in range.
    ...
    def _hash(self, bases: Tensor, positions: Tuple[int, ...]) -> Tensor:
        # Base-4 hash, most-significant digit first, matching `&hashseq` / `tr/ACGT/0123/`.
