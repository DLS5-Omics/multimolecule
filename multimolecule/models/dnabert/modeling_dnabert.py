...
class DnaBertEncoder(nn.Module):
    def __init__(self, config: DnaBertConfig):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([DnaBertLayer(config, layer_idx=i) for i in range(config.num_hidden_layers)])
...
class DnaBertLayer(GradientCheckpointingLayer):
    def __init__(self, config: DnaBertConfig, layer_idx: int | None = None):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = DnaBertAttention(config, layer_idx=layer_idx, is_causal=config.is_decoder)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = DnaBertAttention(
                config,
        ...
        self.intermediate = DnaBertIntermediate(config)
        self.output = DnaBertOutput(config)
...
class DnaBertAttention(nn.Module):
    def __init__(
        self,
        config: DnaBertConfig,
        ...
        attention_class = DnaBertCrossAttention if is_cross_attention else DnaBertSelfAttention
        ...
        self.output = DnaBertSelfOutput(config)
...
class DnaBertSelfAttention(nn.Module):
    def __init__(
        self,
        config: DnaBertConfig,
        position_embedding_type: str | None = None,
...
class DnaBertCrossAttention(nn.Module):
    def __init__(
        self,
        config: DnaBertConfig,
        position_embedding_type: str | None = None,
...
class DnaBertSelfOutput(nn.Module):
    def __init__(self, config: DnaBertConfig):
        super().__init__()
...
class DnaBertIntermediate(nn.Module):
    def __init__(self, config: DnaBertConfig):
        super().__init__()
...
DnaBertPreTrainedModel._can_record_outputs = {
    "hidden_states": DnaBertLayer,
    "attentions": OutputRecorder(DnaBertAttention, index=1, layer_name="attention"),
    "cross_attentions": OutputRecorder(DnaBertAttention, index=1, layer_name="crossattention"),
}
