...
from multimolecule.modules.criterions import BCEWithLogitsLoss

from ..modeling_outputs import ContactPredictorOutput, SequencePredictorOutput, TokenPredictorOutput
from .configuration_unirna import UniRnaConfig, UniRnaSecondaryStructureHeadConfig


class UniRnaPreTrainedModel(PreTrainedModel):
    """
    ...
    base_model_prefix = "unirna"
    supports_gradient_checkpointing = True
    _no_split_modules = ["UniRnaLayer", "UniRnaEmbeddings"]
...
class UniRnaModel(UniRnaPreTrainedModel):
    """
    Examples:
        >>> import torch
        >>> from multimolecule import UniRnaConfig, UniRnaModel, RnaTokenizer
        >>> config = UniRnaConfig()
    ...
    def __init__(self, config: UniRnaConfig, add_pooling_layer: bool = True):
        super().__init__(config)
        self.pad_token_id = config.pad_token_id
        self.embeddings = UniRnaEmbeddings(config)
...
class UniRnaForSequencePrediction(UniRnaPreTrainedModel):
    """
    Examples:
        >>> import torch
        >>> from multimolecule import UniRnaConfig, UniRnaForSequencePrediction, RnaTokenizer
        >>> config = UniRnaConfig()
        >>> model = UniRnaForSequencePrediction(config)
        >>> tokenizer = RnaTokenizer.from_pretrained("multimolecule/rna")
        >>> input = tokenizer("ACGUN", return_tensors="pt")
    ...
    def __init__(self, config: UniRnaConfig):
        super().__init__(config)
        self.unirna = UniRnaModel(config)
...
class UniRnaForTokenPrediction(UniRnaPreTrainedModel):
    """
    Examples:
        >>> import torch
        >>> from multimolecule import UniRnaConfig, UniRnaForTokenPrediction, RnaTokenizer
        >>> config = UniRnaConfig()
        >>> model = UniRnaForTokenPrediction(config)
        >>> tokenizer = RnaTokenizer.from_pretrained("multimolecule/rna")
        >>> input = tokenizer("ACGUN", return_tensors="pt")
...
class UniRnaForContactPrediction(UniRnaPreTrainedModel):
    """
    Examples:
        >>> import torch
        >>> from multimolecule import UniRnaConfig, UniRnaForContactPrediction, RnaTokenizer
        >>> config = UniRnaConfig()
        >>> model = UniRnaForContactPrediction(config)
        >>> tokenizer = RnaTokenizer.from_pretrained("multimolecule/rna")
        >>> input = tokenizer("ACGUN", return_tensors="pt")
...
class UniRnaForMaskedLM(UniRnaPreTrainedModel):
    """
    Examples:
        >>> import torch
        >>> from multimolecule import UniRnaConfig, UniRnaForMaskedLM, RnaTokenizer
        >>> config = UniRnaConfig()
        >>> model = UniRnaForMaskedLM(config)
        >>> tokenizer = RnaTokenizer.from_pretrained("multimolecule/rna")
        >>> input = tokenizer("ACGUN", return_tensors="pt")
    ...
    def __init__(self, config: UniRnaConfig):
        ...
        if config.is_decoder:
            warn(
                "If you want to use `UniRnaForMaskedLM` make sure `config.is_decoder=False` for "
...
class UniRnaForPreTraining(UniRnaForMaskedLM):
    pass


class UniRnaForSecondaryStructurePrediction(UniRnaPreTrainedModel):
    """
    Examples:
        >>> import torch
        >>> from multimolecule import UniRnaConfig, UniRnaForSecondaryStructurePrediction, RnaTokenizer
        >>> config = UniRnaConfig()
        >>> model = UniRnaForSecondaryStructurePrediction(config)
        >>> tokenizer = RnaTokenizer.from_pretrained("multimolecule/rna")
        >>> input = tokenizer("ACGUN", return_tensors="pt")
        >>> output = model(**input)
        >>> output["logits"].shape
    ...
    def __init__(self, config: UniRnaConfig):
        super().__init__(config)
        self.unirna = UniRnaModel(config, add_pooling_layer=False)
        self.ss_head = UniRnaSecondaryStructurePredictionHead(config)
...
class UniRnaEmbeddings(nn.Module):
    """
    ...
    def forward(
        ...
        if self.token_dropout:
            ...
            mask_ratio_train = 0.15 * 0.8  # Hardcoded as the ratio used in all Uni-RNA model training runs
...
class UniRnaLayer(nn.Module):
    def __init__(self, config: UniRnaConfig):
        ...
        self.seq_len_dim = 1
        self.attention = UniRnaAttention(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = UniRnaAttention(config, position_embedding_type="absolute")
        self.layer_norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class UniRnaAttention(nn.Module):
    def __init__(self, config: UniRnaConfig, position_embedding_type: str | None = None):
        super().__init__()
        self.self = UniRnaSelfAttention(config, position_embedding_type=position_embedding_type)
...
class UniRnaPooler(nn.Module):
    ...
    def __call__(self, hidden_states: Tensor, attention_mask: torch.BoolTensor | None = None):
        if attention_mask is None:
            return hidden_states.mean(1)
        return hidden_states.masked_fill(~attention_mask.unsqueeze(-1), 0).mean(1)


@HEADS.contact.logits.register("unirna")
class UniRnaSecondaryStructurePredictionHead(BasePredictionHead):

    config: UniRnaSecondaryStructureHeadConfig
    require_attentions: bool = False
    output_name: str = "last_hidden_state"

    def __init__(self, config: UniRnaConfig, head_config: UniRnaSecondaryStructureHeadConfig | None = None):
        if head_config is None:
            head_config = (
                UniRnaSecondaryStructureHeadConfig(config.head, num_labels=1)
                if config.head is not None
                else UniRnaSecondaryStructureHeadConfig()
            )
        ...
        self.criterion = BCEWithLogitsLoss(self.config)
    ...
    def forward(  # type: ignore[override]  # pylint: disable=arguments-renamed
        ...
        contact_map = self.linear(contact_map)
