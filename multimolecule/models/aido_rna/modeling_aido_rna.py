...
class AidoRnaLayer(nn.Module):
    def __init__(self, config: AidoRnaConfig):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = AidoRnaAttention(config, position_embedding_type="absolute")
        self.layer_norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class AidoRnaAttention(nn.Module):
    def __init__(self, config: AidoRnaConfig, position_embedding_type: str | None = None):
        super().__init__()
        self.self = AidoRnaSelfAttention(config, position_embedding_type=position_embedding_type)
...
class AidoRnaSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in AidoRnaModel forward() function)
...
class AidoRnaSecondaryStructurePredictionHead(BasePredictionHead):
    ...
    def __init__(self, config: AidoRnaConfig, head_config: AidoRnaSecondaryStructureHeadConfig | None = None):
        ...
        self.projection = nn.Linear(config.hidden_size * 2, head_config.channels)
        self.convnet = AidoRnaConvNet(head_config)
        self.prediction = nn.Conv2d(head_config.channels, 1, kernel_size=head_config.kernel_size, padding="same")
    ...
    def forward(  # type: ignore[override]  # pylint: disable=arguments-renamed
        ...
        contact_map = contact_map.permute(0, 3, 1, 2)
    ...
    def symmetrize(self, contact_map: Tensor) -> Tensor:
        """
        Symmetrize a contact_map by copying the upper triangular part to the lower triangular part.

        Args:
            contact_map: Input tensor of shape (..., N, N) where the last two dimensions
                   represent the contact_map to be symmetrized.

        Returns:
            Symmetric tensor of the same shape where lower[i,j] = upper[j,i]
            and diagonal elements are preserved from the original contact_map.
        """
        upper_triangular = torch.triu(contact_map, diagonal=1)
...
class AidoRnaConvNet(nn.Sequential):
    ...
    def __init__(self, config: AidoRnaSecondaryStructureHeadConfig):
        super().__init__(
            *[
                AidoRnaConvLayer(config.channels, config.kernel_size, activation=config.activation, bias=config.bias)
...
class AidoRnaConvLayer(nn.Module):
    ...
    def __init__(self, channels: int, kernel_size: int = 3, activation: str = "relu", bias: bool = False):
        super().__init__()
    ...
    def forward(self, hidden_states: Tensor) -> Tensor:
        ...
        hidden_states = self.activation3(hidden_states)
