...
def _convert_branch(
    ...
    with h5py.File(file, "r") as f:
        ...
        # ChromBPNet is always single-task; multi-task checkpoints are not supported.
        if num_tasks != 1:
            raise ValueError(
                f"ChromBPNet conversion only supports single-task checkpoints (num_tasks=1); got num_tasks={num_tasks}."
            )
        layer = f"{head_prefix}prof_out_precrop"
        kernel = _read_weight(weights, layer, "kernel").permute(2, 1, 0)
        state_dict[f"{prefix}.profile.0.weight"] = kernel
        state_dict[f"{prefix}.profile.0.bias"] = _read_weight(weights, layer, "bias")

        # Count branch: Keras Dense kernel (in, out) -> torch Linear (out, in).
        layer = f"{head_prefix}logcount_predictions"
        state_dict[f"{prefix}.count.0.weight"] = _read_weight(weights, layer, "kernel").t()
        state_dict[f"{prefix}.count.0.bias"] = _read_weight(weights, layer, "bias")
