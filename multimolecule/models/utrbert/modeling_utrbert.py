...
class UtrBertPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: nn.Module):
        ...
        if isinstance(module, UtrBertEmbeddings):
...
class UtrBertEncoder(nn.Module):
    def __init__(self, config: UtrBertConfig):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([UtrBertLayer(config, layer_idx=i) for i in range(config.num_hidden_layers)])
...
class UtrBertLayer(GradientCheckpointingLayer):
    def __init__(self, config: UtrBertConfig, layer_idx: int | None = None):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = UtrBertAttention(config, layer_idx=layer_idx, is_causal=config.is_decoder)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise RuntimeError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = UtrBertAttention(
                config,
...
class UtrBertAttention(nn.Module):
    def __init__(
        ...
        attention_class = UtrBertCrossAttention if is_cross_attention else UtrBertSelfAttention
...
class UtrBertCrossAttention(nn.Module):
    def __init__(
        self,
        config: UtrBertConfig,
        position_embedding_type: str | None = None,
...
UtrBertPreTrainedModel._can_record_outputs = {
    "hidden_states": UtrBertLayer,
    "attentions": OutputRecorder(UtrBertAttention, index=1, layer_name="attention"),
    "cross_attentions": OutputRecorder(UtrBertAttention, index=1, layer_name="crossattention"),
}
