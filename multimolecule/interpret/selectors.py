...
_ATTENTION_WRAPPER_SUFFIXES = (".attention.output", ".crossattention.output")
_ATTENTION_PROJECTION_SUFFIXES = (
    ".self_attn.out_proj",
    ".self_attn.o_proj",
    ".attn.out_proj",
    ".attn.o_proj",
...
_MLP_OUTPUT_SUFFIXES = (".mlp", ".ffn", ".feed_forward")
...
def _select_attention_output_modules(named_modules: dict[str, nn.Module]) -> list[str]:
    wrapper_outputs = [name for name in named_modules if name and name.endswith(_ATTENTION_WRAPPER_SUFFIXES)]
    if wrapper_outputs:
        return wrapper_outputs
    return [name for name in named_modules if name and name.endswith(_ATTENTION_PROJECTION_SUFFIXES)]


def _select_mlp_output_modules(named_modules: dict[str, nn.Module]) -> list[str]:
    return [name for name in named_modules if name and _is_mlp_output(name)]


def _is_mlp_output(name: str) -> bool:
    if name.endswith(_MLP_OUTPUT_SUFFIXES):
        return True
    return name.endswith(".output") and ".attention.output" not in name and ".crossattention.output" not in name
