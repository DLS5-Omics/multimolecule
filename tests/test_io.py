...
def test_dbn_roundtrip(tmp_path) -> None:
    record = io.StructureRecord(sequence="ACGU", dot_bracket="(())", id="test")
...
def test_bpseq_roundtrip(tmp_path) -> None:
    record = io.StructureRecord(sequence="ACGU", dot_bracket="(())")
...
def test_fasta_roundtrip(tmp_path) -> None:
    record = io.StructureRecord(sequence="ACGU", dot_bracket="(())", id="rna1")
...
def test_fasta_plain_sequence(tmp_path) -> None:
    ...
    assert out.id == "id"
    assert out.dot_bracket is None
    assert out.comment == "some description"


def test_fastas_multiple_records(tmp_path) -> None:
    path = tmp_path / "multi.fa"
    io.write_fastas(
        [
            {"id": "rna1", "sequence": "ACGU"},
            {"id": "rna2", "sequence": "AAAA"},
        ],
        path,
    )
    out = io.read_fasta(path)
    assert [rec.id for rec in out] == ["rna1", "rna2"]
    assert out[0].sequence == "ACGU"


def test_st_roundtrip(tmp_path) -> None:
    record = io.StructureRecord(
        sequence="ACGU",
        dot_bracket="(())",
        id="rna1",
        page_number=1,
...
def test_sta_multiple_records(tmp_path) -> None:
    records = [
        ...
        io.StructureRecord(
            ...
            id="rna2",
            page_number=1,
            structure_array="EEEE",
            knot_array="NNNN",
        ),
    ]
    path = tmp_path / "multi.sta"
    io.write_st(records, path)
    out = io.read_st(path)
    assert [rec.id for rec in out] == ["rna1", "rna2"]
    assert out[1].dot_bracket == "...."
