...
def _sequence_pipeline(model=None) -> RegulatorySequencePredictionPipeline:
    pipeline = object.__new__(RegulatorySequencePredictionPipeline)
    pipeline.tokenizer = _DummyTokenizer()
    pipeline.model = model or _DummyModel({0: "cell_a", 1: "cell_b"})
    pipeline.task = "regulatory-sequence-prediction"
    return pipeline


def _track_pipeline() -> RegulatoryTrackPredictionPipeline:
    pipeline = object.__new__(RegulatoryTrackPredictionPipeline)
    pipeline.tokenizer = _DummyTokenizer()
    pipeline.model = _DummyModel({0: "track_a", 1: "track_b"})
    pipeline.task = "regulatory-track-prediction"
    return pipeline


def _profile_pipeline() -> RegulatoryProfilePredictionPipeline:
    pipeline = object.__new__(RegulatoryProfilePredictionPipeline)
    pipeline.tokenizer = _DummyTokenizer()
    pipeline.model = _ProfileModel()
    pipeline.task = "regulatory-profile-prediction"
    return pipeline
...
def test_regulatory_sequence_pipeline_reports_sequence_level_scores():
    pipeline = _sequence_pipeline()
    model_outputs = {
...
def test_regulatory_sequence_pipeline_uses_model_postprocess_and_channels():
    model = _PostprocessModel()
    pipeline = _sequence_pipeline(model)
    model_outputs = {
        "input_ids": torch.tensor([[0, 1, 2]], dtype=torch.long),
        "attention_mask": torch.tensor([[1, 1, 1]], dtype=torch.long),
        "logits": torch.tensor([[1.0, 2.0]]),
...
def test_regulatory_sequence_pipeline_passes_features_to_feature_models_only():
    feature_model = _FeatureModel()
    pipeline = _sequence_pipeline(feature_model)
    ...
    pipeline = _sequence_pipeline(plain_model)
...
def test_regulatory_pipelines_are_registered():
    assert PIPELINE_REGISTRY.check_task("regulatory-sequence-prediction")[0] == "regulatory-sequence-prediction"
    assert PIPELINE_REGISTRY.check_task("regulatory-track-prediction")[0] == "regulatory-track-prediction"
    assert PIPELINE_REGISTRY.check_task("regulatory-profile-prediction")[0] == "regulatory-profile-prediction"
...
def test_regulatory_pipeline_defaults_are_transformers_compatible():
    sequence_task = PIPELINE_REGISTRY.check_task("regulatory-sequence-prediction")[1]
    track_task = PIPELINE_REGISTRY.check_task("regulatory-track-prediction")[1]
    profile_task = PIPELINE_REGISTRY.check_task("regulatory-profile-prediction")[1]
    variant_task = PIPELINE_REGISTRY.check_task("regulatory-variant-effect")[1]

    assert sequence_task["default"]["model"] == ("multimolecule/basset", "main")
    assert track_task["default"]["model"] == ("multimolecule/enformer", "main")
    assert profile_task["default"]["model"] == ("multimolecule/bpnet", "main")
    assert variant_task["default"]["model"] == ("multimolecule/aparent2", "main")
...
def test_regulatory_auto_classes_keep_domain_specific_heads():
    ...
    assert isinstance(AutoModelForRegulatorySequencePrediction.from_config(basset_config), BassetForSequencePrediction)
    assert isinstance(
