...
from multimolecule.pipelines.splicing import SpliceSitePredictionPipeline, SpliceVariantEffectPipeline
...
class _DummyPangolinModel(_DummyModel):
    ...
    def postprocess(self, outputs):
        return outputs, ["heart_no_splice", "heart_splice_site", "heart_usage"]


def _site_pipeline(model_type: str = "openspliceai") -> SpliceSitePredictionPipeline:
    pipeline = object.__new__(SpliceSitePredictionPipeline)
    pipeline.tokenizer = _DummyTokenizer()
    pipeline.model = _DummyPangolinModel() if model_type == "pangolin" else _DummyModel(model_type)
    pipeline.task = "splice-site-prediction"
...
def test_splicing_pipelines_are_registered():
    assert PIPELINE_REGISTRY.check_task("splice-site-prediction")[0] == "splice-site-prediction"
...
def test_splicing_pipeline_defaults_are_transformers_compatible():
    site_task = PIPELINE_REGISTRY.check_task("splice-site-prediction")[1]
...
def test_splicing_pipeline_runs_parameter_free_maxentscan_model():
    ...
    predictor = pipeline("splice-site-prediction", model=model, tokenizer=tokenizer)
