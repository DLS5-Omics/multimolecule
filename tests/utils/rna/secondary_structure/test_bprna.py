...
from multimolecule.utils.rna.secondary_structure import StemEdgeType, bprna
...
def test_segment_topology_graph_helpers() -> None:
    ...
    assert empty_topology.topology.nested_stem_edges == []
    ...
    assert segment_topology.topology.nested_stem_edges
    assert all(isinstance(edge.type, StemEdgeType) for edge in segment_topology.topology.nested_stem_edges)
